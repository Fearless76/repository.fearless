...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.untouchables.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.untouchables.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.untouchables.artwork)') else setting('appearance.alt')
    if appearance == "Untouchables": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.untouchables.artwork)')
